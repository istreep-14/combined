...
function fetchJsonBatchWithEtag(urls, etags) {
  ...
  while (attempts < HTTP_MAX_RETRIES) {
    ...
    try {
      ...
      if (/Bandwidth quota exceeded/i.test(msg) && attempts < 2) { Utilities.sleep(waitMs); continue; }
      // Return per-url error fallback
      var outErr = [];
      for (var k = 0; k < urls.length; k++) outErr.push({ code: 0, status: 'error', error: 'FETCHALL_EXCEPTION', etag: etags ? etags[k] : null, lastModified: null });
      return outErr;
    }
    var out = []; var retryable = false;
    for (var j = 0; j < responses.length; j++) {
      var resp = responses[j];
      var code = 0; try { code = resp.getResponseCode(); } catch (x) { code = 0; }
      var hdrs = {}; try { hdrs = resp.getAllHeaders(); } catch (x2) { hdrs = {}; }
      ...
      else if (code === 429 || (code >= 500 && code < 600)) { retryable = true; out.push({ code: code, status: 'error', error: 'HTTP_' + code, etag: newEtag, lastModified: lastModified }); }
  ...
  // Give up
  var fail = [];
  for (var z = 0; z < urls.length; z++) fail.push({ code: 0, status: 'error', error: 'RETRY_EXHAUSTED', etag: etags ? etags[z] : null, lastModified: null });
  return fail;
}
